...
class Buffer {
 public:
  ...
  const char *beginWrite() const noexcept { return data() + writerIndex_; }

  void hasWritten(size_t len) noexcept;
  const char *peek() const noexcept { return data() + readerIndex_; }
 ...
 private:
  ...
  const char *data() const noexcept { return this->buffer_.data(); };
