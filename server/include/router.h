...
private:
  ...
  void handleCaching(const std::filesystem::path& filePath, const HttpRequest& req, HttpResponse* resp) const;
  size_t splitPath(std::string_view path, std::string_view* segments) const noexcept;
  bool serveStaticFile(const std::filesystem::path& staticFilePath, const HttpRequest& req, HttpResponse* resp) const;
