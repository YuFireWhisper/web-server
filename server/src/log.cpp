...
namespace {
const std::map<LogLevel, std::string> LOG_COLORS = {
    {LogLevel::TRACE, "\033[36m"}, {LogLevel::DEBUG, "\033[34m"},
    {LogLevel::INFO, "\033[32m"},  {LogLevel::WARN, "\033[33m"},
    {LogLevel::ERROR, "\033[31m"}, {LogLevel::FATAL, "\033[1;31m"}};
...
std::string Logger::formatLogMessage(const LogEntry& entry) {
  ...
  return std::format("{}{} [{}] {}:{} - {}\033[0m", getLevelColor(entry.level),
                     timestamp, getLevelString(entry.level), entry.file,
